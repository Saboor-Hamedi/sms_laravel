                ...
                <article class="front-card">
                    <header class="front-card-header">
                        <div class="front-card-profile">
                            ...
                            @else
                                <img src="{{ asset('storage/default/default-profile.png') }}"
                                    alt="Default Profile" class="front-profile-img">
                            @endif

                            <div class="front-card-author">
                                <span class="front-author-name">{{ Str::ucfirst($post->user->name ?? 'Anonymous') }}</span>
                                <span class="text-sm front-author-time">{{ $post->created_at->setTimezone('UTC')->diffForHumans() }}</span>
                            </div>
                        </div>
                    ...
                    <footer class="front-card-footer">
                        ...
                        <div class="front-card-actions">
                            <a href="{{ route('post.show', $post->slug ?? $post->id) }}"
                                class="default-button" style="padding: 8px !important; font-size: 12px">
                                Read More
                            </a>
                            @auth
